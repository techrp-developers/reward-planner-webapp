...
export const MetricsSocialProofSection = () => {
  return (
    <section className="bg-white rounded-3xl border border-gray-200/80 p-5 sm:p-7 shadow-xs relative overflow-hidden">
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-1.5">
          <span className="inline-block px-3 py-1 rounded-full bg-violet-50 border border-violet-200/70 text-xs font-bold uppercase tracking-wider text-[#7C3AED]">
        ...
        {/* 4 Metric Cards */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 sm:gap-5 pt-1">
          {METRICS.map((metric) => {
            const Icon = metric.icon;
            return (
              <div
                key={metric.id}
                className={`p-4 sm:p-5 rounded-2xl border ${metric.border} bg-gradient-to-b from-white to-gray-50/50 hover:shadow-md transition-all duration-200 flex flex-col justify-between group`}
              >
                <div className="flex items-center justify-between mb-3">
                  <div
                    className={`w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-br ${metric.accent} flex items-center justify-center transition-transform group-hover:scale-105`}
                  >
                    <Icon sx={{ fontSize: 22 }} />
                  </div>
